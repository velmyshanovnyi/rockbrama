...
<div class="modals">
	<?php if (isset($_GET['login']) and !$logged) { ?>
		<div class="modal modal-login">
			<a class="close-btn" href="#" onclick="document.location.search='';"> </a>
		...
		<script type="text/javascript">
			jQuery(function($) {
				$(".overlay").show();
				$(".modal-login").css({'position': 'fixed', 'z-index': '10000', 'left': '50%', 'top': '300px'}).show().css({'margin-left': '-' + $(".modal-login").outerWidth() / 2 + 'px'});
			});
		</script>
	<?php } ?>
	<?php if (isset($_GET['change_password']) and $logged) { ?>
		...
		<script type="text/javascript">
			jQuery(function($) {
				$(".overlay").show();
				$(".modal-change-pwd").css({'position': 'fixed', 'z-index': '10000', 'left': '50%', 'top': '300px'}).show().css({'margin-left': '-' + $(".modal-change-pwd").outerWidth() / 2 + 'px'});
			});
		</script>
	<?php } ?>
</div>